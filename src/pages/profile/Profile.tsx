...
import { fetchProfileListSelector } from "@/recoil/atoms/profile.ts";
...
const Profile = () => {
  ...
  // recoil
  const profileLoadable = useRecoilValueLoadable(
    fetchProfileListSelector({
      type: "profile",
      search: searchText,
    }),
  );
  ...
  // state
  const list =
    profileLoadable.state === "hasValue" ? profileLoadable.contents : [];
  const isLoading = profileLoadable.state === "loading";
  // page
  ...
  return (
    <PageContainer>
      <PageTitle>프로파일 관리</PageTitle>
      <PageContent
        handleSortChange={setSearchText}
